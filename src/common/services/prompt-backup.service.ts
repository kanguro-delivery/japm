...
export class PromptBackupService {
  ...
  constructor(
    ...
    this.backupDir =
      this.configService.get<string>('PROMPT_BACKUP_DIR') ||
  ...
  async createPromptBackup(
    ...
    try {
      ...
      if (!promptWithRelations) {
        throw new Error(
          `Prompt "${promptId}" not found in project "${projectId}"`,
        );
      }
      ...
      const backupData: PromptBackupDto = {
        ...
        versions: promptWithRelations.versions.map((version) => ({
          ...
          marketplaceRejectionReason:
            version.marketplaceRejectionReason ?? undefined,
        ...
        assets: promptWithRelations.assets.map((asset) => ({
          ...
          versions: asset.versions.map((version) => ({
            ...
            marketplacePublishedAt:
              version.marketplacePublishedAt?.toISOString(),
            marketplaceRequestedAt:
              version.marketplaceRequestedAt?.toISOString(),
            marketplaceApprovedAt: version.marketplaceApprovedAt?.toISOString(),
            marketplaceRejectionReason:
              version.marketplaceRejectionReason ?? undefined,
            translations: version.translations.map((translation) => ({
        ...
        executionLogs: options.includeExecutionLogs
          ? await this.getExecutionLogs(
              promptId,
              options.executionLogsLimit || 50,
            )
          : [],
        statistics,
      };
   ...
   */
  async listBackups(projectId?: string): Promise<
    Array<{
      fileName: string;
      filePath: string;
      size: number;
  ...
  > {
    try {
      ...
      const backupFiles = files.filter(
        (file) => file.startsWith('prompt-backup-') && file.endsWith('.json'),
      );
      ...
      for (const fileName of backupFiles) {
        try {
          ...
          this.logger.warn(
            `Error reading backup file ${fileName}: ${error.message}`,
          );
        }
      }
